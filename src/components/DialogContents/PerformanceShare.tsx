  ...
  return (
    <>
      <div className="flex flex-col gap-2 w-full bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 p-4 rounded-lg">
        <p className="font-bold text-xl text-center text-balance">
      ...
      {canShare ? (
        <button
          type="button"
          onClick={() => {
            void share(shareText, URL, shareText);
            trackSharedResults();
          }}
...
function buildShareText(completedPuzzles: CompletedPuzzle[]): string {
  ...
  SOLUTION_SIZES.map((size: number) => {
    ...
    if (puzzle) {
      out += `\n${strSize}x${strSize} - ${formatDurationMs(puzzle.timeToCompleteMs)} ${puzzle.isThemeRevealed ? "(with theme revealed)" : ""}`;
    }
  });
  return out;
}